...
from module.dropout_wrapper_chr import DropoutWrapper
from bert.modeling import BertConfig, BertEncoder, BertLayerNorm, BertModel
from module.san import SANClassifier, Classifier
import pdb
from transformers import CharacterBertModel

class SANChrBertNetwork(nn.Module):
    def __init__(self, opt, bert_config=None):
        super(SANChrBertNetwork, self).__init__()
        self.dropout_list = []
        self.bert_config = BertConfig.from_dict(opt)
        self.bert = CharacterBertModel.from_pretrained('/content/drive/MyDrive/NLP/new/character-bert/pretrained-models/medical_character_bert/')
    ...
    def freeze_bert(self):
        self.require_grads_bert={}
        for name, p in self.bert.named_parameters():
            self.require_grads_bert[p]=p.requires_grad
            # print(name,': set from',p.requires_grad, 'to False')
            p.requires_grad = False

    def unfreeze_bert(self):
        for name,p in self.bert.named_parameters():
            # print(name,': set from False to ',self.require_grads_bert[p])
            p.requires_grad = self.require_grads_bert[p]
